...
export async function HackerCard ({ user }: { user: User }) {
  let participant: PageObjectResponse
  let judge: PageObjectResponse
  let name: string
  let hackerId: number
  let teamName: string
  ...
  judge = await fetchJudgeNotionPage(user)
  if (judge) {
  ...
  const teammates = teammatesPages.map(
    teammate => {
        ...
        <ul className={styles.teammates}>
          {
            teammates
              ? teammates.map(
                teammate => <li key={teammate.email}>
